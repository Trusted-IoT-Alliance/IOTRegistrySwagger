...
POPCODEBPUBK=0297bb72d581a99abb4b006fac0403871c9ec3b54ce303d2709864752bdf867350
POPCODEBPRIVK=16f8ef102297bdf5c3fddbbf443eb7ff74ba0450b867d2d0bab6b1c826e62250
POPCODEBADDR=4f967c63925015fc9186231d739c6ad185dc0f4c
...
# Generate Unitize Signature
echo Generate Possess Signature
echo "http://hl-sk-j-040.brackets.trade:8081/v1/generateUnitizeSig?CounterSeedStr=$CounterSeedStr&PopcodeDestAddress=$POPCODEBADDR&Output=$Output&DestAmounts=2&Data=1&PopcodeSourcePrivateKey=$POPCODEAPRIVK"

result=$(curl -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' \
"http://hl-sk-j-040.brackets.trade:8081/v1/generateUnitizeSig?CounterSeedStr=$CounterSeedStr&PopcodeDestAddress=$POPCODEBADDR&Output=$Output&DestAmounts=2&Data=1&PopcodeSourcePrivateKey=$POPCODEAPRIVK")
SIG=$(echo "console.log(JSON.parse('$result').sig)" | node)
if [ -z "$SIG" ] ;  then
    echo "did not get signature"
    exit
...
# Unitize
echo Unitize
echo "http://hl-sk-j-040.brackets.trade:8081/v1/unitize?Data=1&DestAddress=$POPCODEBADDR&DestAmounts=1%2C1&SourceAddress=$POPCODEAADDR&SourceOutput=$Output&PopcodePubKey=$POPCODEAADDR&PopcodeSig=$SIG"
result=$(curl -X POST --header 'Content-Type: application/json' --header 'Accept: text/html' \
"http://hl-sk-j-040.brackets.trade:8081/v1/unitize?Data=1&DestAddress=$POPCODEBADDR&DestAmounts=1%2C1&SourceAddress=$POPCODEAADDR&SourceOutput=$Output&PopcodePubKey=$POPCODEAADDR&PopcodeSig=$SIG")
echo $result
